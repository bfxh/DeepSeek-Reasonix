...
def _mask(src: str, js: bool = False, go: bool = False) -> str:
    """把字符串/字符字面量与注释替换成空格（**保留换行**），供花括号配平与声明识别用。

    Go 另有两种会骗过朴素配平的东西：**反引号原始字符串**（`//` 里可以整段带花括号、
    带换行，且**不吃转义**）与 rune 字面量（`'{'`）。两者都必须整体掩掉，否则
    `desktop/app.go` 这种万行文件的括号配平会整体失真。
        ...
        if state is None:
            ...
            if c == "'":
                if js:
                    ...
                    i += 1
                    continue
                if go:                        # Go 没有生命周期 ⇒ `'` 一定是 rune 字面量
                    ln = _char_lit_len(src, i) or 3
                    out.append(" " * ln)
                    i += ln
                    continue
...
def brace_metrics(src: str, js: bool = False, go: bool = False) -> list[tuple[str, int, str]]:
    ...
    """
    src = _mask(src, js=js, go=go)
    out: list[tuple[str, int, str]] = []
    if go:
        # Go：`func` 可以是函数也可以是**带接收者的方法**（`func (r *Foo) Bar()`）；
        # 类型声明是 `type X struct/interface`（成员 = 字段数 / 接口方法数）。
        pat = re.compile(r"^\s*func\s+(?:\([^)]*\)\s+)?([A-Za-z_][A-Za-z0-9_]*)"
                         r"|^\s*type\s+([A-Za-z_][A-Za-z0-9_]*)\s+(?:struct|interface)")
        fn_line = re.compile(r"^\s*func\s")
        ...
        for i, line in enumerate(lines):
            ...
            if kind == "fn":
                ...
                for j in range(i, end + 1):
                    ...
                    if j == i:
                        continue
                    if d0 == 1 and (field_line.match(lines[j]) or fn_line.match(lines[j])):
                        members += 1
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for fn in filenames:
            ...
            metrics = py_metrics(src) if fp.suffix in PY else (
                brace_metrics(src, js=fp.suffix != ".rs" and fp.suffix not in GO,
                              go=fp.suffix in GO) if fp.suffix in BRACE else [])
