...
def split_active(valid):
    """按 expires 分成活跃 / 过期两堆（过期的只提示，不参与域冲突判定）。"""
    today = _today()
    active, expired = [], []
    for fp, doc in valid:
...
def check_overlap(active, tracked, fails):
    """A3：两份**活跃**声明的 scope 有交集 ⇒ 红。"""
...
def check_identity(a, active, fails):
    """A4/A5：设了身份就必须有声明；改动不许落在别人的活跃域里。"""
...
def main() -> int:
    ...
    active, expired = split_active(valid)
    ...
    check_overlap(active, tracked, fails)
    check_identity(a, active, fails)
