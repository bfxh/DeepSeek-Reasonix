...
STEPS = [
    ...
    ("ctxfirst-gate", [PY, "-X", "utf8", G + "ctxfirst_gate.py", "--git-tracked"], "fast",
     "context 首参（context.Context 必须是函数第一个参数，否则整条 ctx 链断了）"),
    ("ctxtodo-gate", [PY, "-X", "utf8", G + "ctxtodo_gate.py", "--git-tracked"], "fast",
     "context.TODO 硬禁止（产品代码禁止半成品 ctx，只允许 Background 作根）"),
    ("grrecover-gate", [PY, "-X", "utf8", G + "grrecover_gate.py", "--git-tracked"], "fast",
     "协程 recover（go func 字面量体内须有 recover 兜底，否则 panic 拖垮进程）"),
    ("ioutil-gate", [PY, "-X", "utf8", G + "ioutil_gate.py", "--git-tracked"], "fast",
     "弃用 ioutil 硬禁止（自 Go 1.16 起整包弃用，改用 os/io/io/fs）"),
    ("errignore-gate", [PY, "-X", "utf8", G + "errignore_gate.py", "--git-tracked"], "fast",
     "忽略错误返回（_ = / x, _ = 丢弃调用结果，errcheck 风格，只拦新增）"),
    ("linelen-gate", [PY, "-X", "utf8", G + "linelen_gate.py", "--git-tracked"], "fast",
     "行长（单行 >200 字符即记，只拦新增超长行）"),
...
WRITE_STEPS = [
    ...
    ("ctxfirst-baseline", [PY, "-X", "utf8", G + "ctxfirst_gate.py", "--git-tracked", "--write"]),
    ("grrecover-baseline", [PY, "-X", "utf8", G + "grrecover_gate.py", "--git-tracked", "--write"]),
    ("errignore-baseline", [PY, "-X", "utf8", G + "errignore_gate.py", "--git-tracked", "--write"]),
